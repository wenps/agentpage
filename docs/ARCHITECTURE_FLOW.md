...
| 工具 | 文件 | 功能 |
|------|------|------|
| `dom` | `web/tools/dom-tool/` | 点击、填写、键入、聚焦、悬停、按键、读属性、改属性、加/删 class；fill 自动穿透 ARIA widget（slider/spinbutton）到关联 input |
...
## 十、系统提示词架构（Decision Framework）

> **v0.0.26 起，`system-prompt.ts` 已重写为 Decision Framework 结构，不再逐条罗列规则。**
> 以下描述对应当前实现。

### 设计理念

旧版 prompt 以 `## Rules` 列出十余条规则，模型需自行推断优先级。
新版改用**五阶段决策流**，与 Agent Loop 的执行阶段一一对应，更易于模型追踪当前处于哪个判断点。

### 决策流（Decision Flow）

```
ANALYZE → ASSESS → CHOOSE → EXECUTE → OUTPUT
```

1. **ANALYZE**：读取当前快照，分析可见元素
2. **ASSESS**：根据当前任务（remaining）评估哪些目标在快照中可见且可操作
3. **CHOOSE**：选择动作类型（click/fill/press/scroll/…），优先批量执行
4. **EXECUTE**：生成 tool_calls
5. **OUTPUT**：返回 `REMAINING: <text>` 或 `REMAINING: DONE`

### Targeting Rules

- 使用快照中的 `#hashID` 作为 selector，不猜测 CSS 选择器
- ordinal（"第N个按钮"）为 1-based 视觉序

### Constraints

- form-input 顺序约束：`focus/click` → `fill/type/select_option`
- DOM 结构变化动作（click 打开弹窗/切换路由等）执行后断轮，等待下一轮新快照
- 禁止冗余 `page_info.snapshot` 调用

### Listener Abbreviations

快照中 `listeners="clk,inp,chg"` 使用缩写，prompt 中提供映射表：
...
prompt 的工具部分（`## Tools`）完全由各工具注册时的 `t.description` 动态注入，正文不再重复工具能力描述。
这消除了 prompt 与工具定义之间的内容冲突，也减少了每轮请求约 150 tokens 的冗余开销。
...
### Demo Prompt 配置示例（Element Plus）

在 Demo 页面（`demo/App.vue`）中，建议注册一条场景化 prompt（key: `demo`）：
...
`web` 入口在模块加载时默认安装监听追踪补丁，用于增强快照对“真实可交互元素”的识别能力：

- 入口位置：`src/web/index.ts`
- 追踪实现：`src/web/event-listener-tracker.ts`
- 核心机制：劫持 `EventTarget.prototype.addEventListener/removeEventListener`
- 记录范围：仅 `Element` 目标（`WeakMap<Element, Set<string>>`）

与快照联动（`src/web/tools/page-info-tool.ts`）：

- 元素行可输出 `listeners="..."` 运行时事件信息
- 交互优先级判定会纳入被追踪事件（如 `click/input/change/keydown`）
- 布局剪枝时，带事件监听的容器不会被错误折叠

边界：

- 事件委托只能定位到绑定监听的父容器，无法直接还原到具体子节点
- 补丁安装前的历史绑定不会回溯记录
...
| `DEFAULT_WAIT_MS` | `1000` | `dom-tool/constants.ts` | DOM 操作前默认等待时间(ms) |
...
| DOM 操作工具 | `src/web/tools/dom-tool/` |
